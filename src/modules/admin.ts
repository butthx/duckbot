    ...
    return replyToMessage(
        ...
        [
          [
            {
              text: langs.pmButton,
              url: `https://t.me/${username}?start=settings_${ctx.chat.id}`,
              hide: true,
            },
          ],
        ],
...
export async function handleSettings(ctx) {
  ...
  try {
    ...
    let value = ctx.message.text.replace(
        new RegExp(`^/start(\@${String(process.env.USERNAME).replace(/^\@/, '').trim()})? settings_`),
        '',
    );
...
export async function reportAdmin(ctx) {
  ...
  try {
    ...
    return replyToMessage(
        ctx,
        `${langs.report}\n⏱ <code>${c}</code> | ⏳ <code>${await getPing(ctx)}</code>${admin}`,
        false,
    );
  } catch (error) {
    return reportError(error, ctx);
