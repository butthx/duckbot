...
export async function purge(event:NewMessageEvent) {
  ...
  try{
    ...
    if(!message.replyTo){
      return bot.telegram.sendMessage(Number(event.chatId),`${langs.mustReply}\n⏱ <code>${c}</code> | ⏳ <code>${await gramGetPing(event)}</code>`,{
        parse_mode : "HTML",
    ...
    if(event.isPrivate){
      ...
      if(!admin){
        return bot.telegram.sendMessage(Number(event.chatId),`${langs.userNonAdmin}\n⏱ <code>${c}</code> | ⏳ <code>${await gramGetPing(event)}</code>`,{
        parse_mode : "HTML",
        reply_to_message_id : msgId
      })
      }
      try{
        let res:Api.messages.AffectedMessages = await client.invoke(new Api.channels.DeleteMessages({
          channel : message.inputChat,
          id : arr
        }))
      }catch(e){}
    }
    return bot.telegram.sendMessage(Number(event.chatId),`${langs.purgeSuccess}\n⏱ <code>${c}</code> | ⏳ <code>${await gramGetPing(event)}</code>`,{
        parse_mode : "HTML"
    })
  }catch(error){
