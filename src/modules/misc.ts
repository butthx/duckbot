  ...
  try{
    if("message" in ctx.update){
      ...
      if(msg.chat.type == "private"){
        let data = await privates.findOne({chat_id:chat_id})
     ...
     for(let i = 0; i < match.length; i++){
      ...
      if(groups.same){
        if(keyboard.length == 0){
          let rows = new Array()
          rows.push({text:btnText,url:btnUrl,hide:true})
          if (rows.length >= 1) {
               keyboard.push(rows)
          }else {
               keyboard.push(rows)
           ...
           keyboard[num].push({text:btnText,url:btnUrl,hide:true})
        }
      }else{
        let rows = new Array()
          rows.push({text:btnText,url:btnUrl,hide:true})
...
export function reportError(err,ctx){
  try {
    ...
    console.log(ctx.telegram.sendDocument(Number(process.env.ERROR_LOG),{
...
export function isIso(lang){
  try{
    let arr = ["af","sq","am","ar","hy","az","eu","be","bn","bs","bg","ca","ceb","ny","zh-cn","zh-tw","co","hr","cs","da","nl","en","eo","et","tl","fi","fr","fy","gl","ka","de","el","gu","ht","ha","haw","iw","hi","hmn","hu","is","ig","id","ga","it","ja","jw","kn","kk","km","ko","ku","ky","lo","la","lv","lt","lb","mk","mg","ms","ml","mt","mi","mr","mn","my","ne","no","ps","fa","pl","pt","pa","ro","ru","sm","gd","sr","st","sn","sd","si","sk","sl","so","es","su","sw","sv","tg","ta","te","th","tr","uk","ur","uz","vi","cy","xh","yi","yo","zu"]
