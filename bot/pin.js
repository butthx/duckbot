  ...
  checkpin: async function(ctx) {
    try {
      ...
      if (ctx.message.chat.username === undefined) {
        ...
        let url = `https://t.me/c/${spl}/${msg_id}`;
        pesan += `\n<code>${new Date().toUTCString()}</code>`;
        return util.kirimPesanPin(ctx, pesan, [
          [{ text: `📌 ${await util.getLang(ctx, "newPinButton")}`, url: url }]
        ]);
      } else {
    ...
    try {
      ...
      if (ctx.message.reply_to_message) {
        if (rpl.toLowerCase() == "loud") {
          return ctx.pinChatMessage(ctx.message.reply_to_message.message_id, {
        ...
        return util.kirimPesanPin(
          ctx,
          `${await util.getLang(ctx, "mustReply")}`
        );
      }
    } catch (error) {
      console.log(error);
  ...
  unpinMessage: async function(ctx) {
    try {
      ...
      if (getChat.pinned_message == undefined) {
        return util.kirimPesanPin(ctx, `${util.getLang(ctx, "noPin")}`);
      }
      let botAdmin = report_module.adminCheck(ctx, ctx.botInfo.id);
      ...
      if (rpl.toLowerCase() == "all") {
        return util.kirimPesanPin(
          ctx,
          `${await util.getLang(ctx, "unpinAll")}`,
          [
            [
              {
                text: await util.getLang(ctx, "yes"),
                callback_data: "unpinall yes"
              },
              {
                text: await util.getLang(ctx, "no"),
